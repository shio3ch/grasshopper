...
1. **Hacker News トップストーリー**（JSON API）
   ...
   - 各 ID について `https://hacker-news.firebaseio.com/v0/item/{id}.json` を WebFetch で取得する。
   - 各アイテムの `title` と `url` フィールドを記事に使用する。

2. **Zenn トレンド**
   - `https://zenn.dev/api/articles?order=trending&count=10` を WebFetch で取得する。
   - `articles` 配列の各要素から `title` と `path`（`https://zenn.dev` + `path`）を使用する。

3. **GitHub Blog**
   - `https://github.blog/feed/` を WebFetch で取得する（RSS/Atom 形式）。
   - 各エントリの記事タイトルと URL を抽出する。
